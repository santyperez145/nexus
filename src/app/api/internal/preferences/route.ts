...
export async function POST(req: Request) {
  ...
  if (booleanFields.some((field) => body[field] != null && typeof body[field] !== "boolean")) {
    return Response.json({ error: "Preference flags must be booleans" }, { status: 400 });
  }
  const threshold =
  ...
  const lowBalanceThreshold =
    body.lowBalanceThresholdUsd != null ? Number(body.lowBalanceThresholdUsd) : undefined;
  if (
    ...
    (lowBalanceThreshold != null &&
      (!Number.isFinite(lowBalanceThreshold) || lowBalanceThreshold < 0.01 || lowBalanceThreshold > 10_000))
  ) {
    return Response.json(
      {
        error:
  ...
  const result = await applyPreferenceUpdate(session.user.id, {
    ...
    autoTopupEnabled: body.autoTopupEnabled != null ? body.autoTopupEnabled : undefined,
    autoTopupThresholdUsd: threshold != null ? String(threshold) : undefined,
    autoTopupAmountUsd: amount != null ? String(amount) : undefined,
    notifyLowBalance: body.notifyLowBalance != null ? body.notifyLowBalance : undefined,
    notifyKeyLimit: body.notifyKeyLimit != null ? body.notifyKeyLimit : undefined,
    notifyOrgInvite: body.notifyOrgInvite != null ? body.notifyOrgInvite : undefined,
    lowBalanceThresholdUsd:
      lowBalanceThreshold != null ? String(lowBalanceThreshold) : undefined,
  });
  return Response.json({ ok: true, data: result.privacy, purged: result.purged });
}
