...
export function ModelAccessButton({ path }: { path: string }) {
    ...
    <div className="flex flex-wrap items-center gap-3">
      <Button
        ...
        onClick={async () => {
          ...
          setMessage(null);
          try {
            const response = await fetch(`/api/v1/models/${path}/access`, { method: "POST" });
            const json = (await response.json().catch(() => ({}))) as {
              data?: { status?: string };
              error?: { message?: string };
            };
            if (!response.ok) throw new Error(json.error?.message ?? "No se pudo solicitar acceso");
            setState("pending");
            setMessage(
              json.data?.status === "owner" ? "Ya tenés acceso." : "Solicitud enviada al propietario.",
            );
          } catch (error) {
            setState("error");
